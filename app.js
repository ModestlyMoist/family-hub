...
function mealHistoryView(){let current=weekStart(),weeks=Array.from({length:8},(_,i)=>{let d=new Date(current+'T12:00');d.setDate(d.getDate()-(i+1)*7);let dates=habitDays(iso(d));return {dates,chosen:dates.filter(ds=>data.mealPlan[ds])}});return `<section class="card full"><div class="card-head"><div><h2>Meals</h2><div class="meta">Previous weekly selections</div></div></div><div class="meal-view-tabs"><button class="meal-view-btn" data-mode="plan">Plan</button><button class="meal-view-btn active" data-mode="history">History</button></div></section><section class="card full"><div class="meal-history">${weeks.map(w=>`<div class="meal-history-week"><div class="card-head"><div><b>${new Date(w.dates[0]+'T12:00').toLocaleDateString([],{month:'short',day:'numeric'})} – ${new Date(w.dates[6]+'T12:00').toLocaleDateString([],{month:'short',day:'numeric',year:'numeric'})}</b><div class="meta">${w.chosen.length} meals planned</div></div>${w.chosen.length?`<button class="mini reuse-meal-week" data-start="${w.dates[0]}">Use this week again</button>`:''}</div>${w.chosen.map(ds=>`<div class="meal-history-row"><span>${new Date(ds+'T12:00').toLocaleDateString([],{weekday:'short'})}</span><b>${data.mealPlan[ds]}</b></div>`).join('')||'<p class="muted">No meals saved for this week.</p>'}</div>`).join('')}</div></section>`}
