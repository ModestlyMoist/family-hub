...
function tasksView(){let all=data.tasks.filter(t=>taskFilter==='All'||(t.assigned||'Everyone')===taskFilter),house=all.filter(t=>t.category==='Household'||t.category==='Chore'),school=all.filter(t=>!house.includes(t)),due=house.filter(t=>t.taskMode==='cycle'&&choreStatus(t).startsWith('Due now')),upcoming=house.filter(t=>t.taskMode==='cycle'&&!choreStatus(t).startsWith('Due now')),datedHouse=house.filter(t=>t.taskMode!=='cycle'),rows=arr=>arr.map(t=>`<div class="item"><input class="check task-check" data-id="${t.id}" type="checkbox" ${t.done?'checked':''}><div style="flex:1"><b>${t.name}</b><div class="meta">${choreStatus(t)}${t.category?` · ${t.category}`:''} · ${t.assigned||'Everyone'}</div></div><div class="item-actions"><button class="mini edit-task" data-id="${t.id}">Edit</button><button class="mini danger delete-task" data-id="${t.id}">Delete</button></div></div>`).join('');return `<section class="card wide"><div class="card-head"><div><h2>Tasks</h2><div class="meta">Chores, school, activities & reminders</div></div><button id="addTask">＋ Task</button></div><div class="actions" style="margin-top:12px">${['All','Richard','Timothy','Olivia','Everyone'].map(x=>`<button class="task-filter ${taskFilter===x?'primary':''}" data-filter="${x}">${x}</button>`).join('')}</div></section><section class="card"><div class="card-head"><div><h2>Household</h2><div class="meta">${due.length} due now · ${upcoming.length} upcoming</div></div></div><h3>Due now</h3>${rows(due)||'<p class="muted">Nothing due right now 🎉</p>'}<h3 style="margin-top:18px">Upcoming</h3>${rows(upcoming)||'<p class="muted">No upcoming recurring chores.</p>'}${datedHouse.length?`<h3 style="margin-top:18px">Other household tasks</h3>${rows(datedHouse)}`:''}</section><section class="card"><h2>School & Activities</h2>${rows(school)||'<p class="muted">No school or activity tasks for this filter.</p>'}</section>`}
