...
function addMealIngredientsToShopping(m){let ingredients=(m.ingredients||'').split(',').map(x=>x.trim()).filter(Boolean);if(!ingredients.length){alert('This meal has no ingredients yet.');return}ingredients.forEach(name=>{let existing=data.staples.find(s=>s.name.toLowerCase()===name.toLowerCase());if(existing)existing.need=true;else data.staples.push({id:Date.now()+Math.random(),name,qty:1,need:true,permanent:false})});save();render()}
