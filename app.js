...
function taskDueOn(t,ds){if(t.taskMode!=='cycle')return t.due===ds;if(t.cycle==='Pick the Days'){let d=new Date(ds+'T12:00');return (t.cycleDays||[]).includes(d.getDay())}let n=choreIntervalDays(t);if(!n)return false;let anchor=t.cycleStart||t.lastCompleted||iso(new Date());let diff=Math.floor((new Date(ds+'T12:00')-new Date(anchor+'T12:00'))/86400000);return diff>=0&&diff%n===0}
function nextTaskDue(t,from=iso(new Date())){if(t.taskMode!=='cycle')return t.due||'';for(let i=0;i<370;i++){let d=new Date(from+'T12:00');d.setDate(d.getDate()+i);let ds=iso(d);if(taskDueOn(t,ds))return ds}return ''}
function choreStatus(t){if(t.taskMode!=='cycle')return t.due?fmtDate(t.due):'No due date';if(t.cycle==='Pick the Days'||t.cycleStart){let nd=nextTaskDue(t);if(nd===iso(new Date()))return 'Due now · '+(t.cycle||'Chore');return nd?'Next: '+fmtDate(nd)+' · '+(t.cycle||'Chore'):'No upcoming date';}let days=choreIntervalDays(t);if(!t.lastCompleted)return 'Due now · '+(t.cycle||'Chore');let last=new Date(t.lastCompleted+'T12:00'),today=new Date(iso(new Date())+'T12:00'),elapsed=Math.floor((today-last)/86400000),left=days-elapsed;if(left<=0)return 'Due now · '+t.cycle;if(elapsed===0)return 'Done today · '+t.cycle;return 'Due in '+left+' day'+(left===1?'':'s')+' · '+t.cycle}
