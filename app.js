...
let data=JSON.parse(localStorage.getItem('familyHubData')||'null')||SAMPLE; let view='home'; let calendarMode='month'; let taskFilter='All';let taskViewMode='tasks';let mealViewMode='plan'; let calCursor=new Date(); let habitCursor=weekStart(); let habitMode='week'; let habitSummaryRange='weekly'; calCursor.setDate(1); let activePin=sessionStorage.getItem('familyHubPin')||localStorage.getItem('familyHubPin')||'';
...
function taskProgressView(){let groups=[{title:'Household',match:t=>t.category==='Household'||t.category==='Chore',cls:'household'},{title:'School + Activities',match:t=>['School','Sports'].includes(t.category),cls:'school'},{title:'Work',match:t=>t.category==='Work',cls:'work'}],dates=habitDays(weekStart());return `<section class="card full"><div class="card-head"><div><h2>Tasks</h2><div class="meta">Completion progress</div></div></div><div class="task-view-tabs"><button class="task-view-btn" data-mode="tasks">Tasks</button><button class="task-view-btn active" data-mode="progress">Progress</button></div></section>${groups.map(g=>{let items=data.tasks.filter(g.match),recent=dates.slice().reverse().flatMap(ds=>Object.keys(data.taskLog?.[ds]||{}).filter(id=>data.taskLog[ds][id]).map(id=>({ds,t:data.tasks.find(t=>t.id==id)}))).filter(x=>x.t&&items.some(t=>t.id==x.t.id)),counts=dates.map(ds=>Object.keys(data.taskLog?.[ds]||{}).filter(id=>data.taskLog[ds][id]&&items.some(t=>t.id==id)).length),max=Math.max(1,...counts);return `<section class="task-area task-area-${g.cls}"><div class="task-area-head"><h2>${g.title}</h2><span class="pill">✓ ${counts.reduce((a,b)=>a+b,0)} this week</span></div><div class="card full"><div class="task-week-bars">${dates.map((ds,i)=>`<div class="task-day-bar"><div class="task-day-track"><i style="height:${Math.max(counts[i]?12:0,counts[i]/max*100)}%"></i></div><b>${counts[i]}</b><span>${new Date(ds+'T12:00').toLocaleDateString([],{weekday:'short'})}</span></div>`).join('')}</div></div></section>`}).join('')}`}
