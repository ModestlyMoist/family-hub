...
function choreIntervalDays(t){return t.cycle==='Daily'?1:t.cycle==='Every 3 Days'?3:t.cycle==='Weekly'?7:t.cycle==='Every 2 Weeks'?14:0}
...
function choreHistory(t){return Object.keys(data.choreLog||{}).filter(ds=>data.choreLog[ds]?.[t.id]).sort().reverse()}
function tasksView(){let all=data.tasks.filter(t=>taskFilter==='All'||(t.assigned||'Everyone')===taskFilter),house=all.filter(t=>t.category==='Household'||t.category==='Chore'),school=all.filter(t=>!house.includes(t)),due=house.filter(t=>t.taskMode==='cycle'&&choreStatus(t).startsWith('Due now')),upcoming=house.filter(t=>t.taskMode==='cycle'&&!t.done&&!choreStatus(t).startsWith('Due now')),recent=Object.entries(data.choreLog||{}).flatMap(([ds,ids])=>Object.keys(ids).filter(id=>ids[id]).map(id=>({ds,t:data.tasks.find(t=>t.id==id)}))).filter(x=>x.t&&(taskFilter==='All'||(x.t.assigned||'Everyone')===taskFilter).sort((a,b)=>b.ds.localeCompare(a.ds)).slice(0,8),rows=arr=>arr.map(t=>`<div class="item chore-row"><input class="check task-check" data-id="${t.id}" type="checkbox" ${t.done?'checked':''}><div style="flex:1"><b>${t.name}</b><div class="meta">${choreStatus(t)} · ${t.assigned||'Everyone'}</div></div><span class="pill">${t.taskMode==='cycle'?(t.cycle||'Repeating'):(t.category||'Task')}</span><div class="item-actions"><button class="mini edit-task" data-id="${t.id}">Edit</button><button class="mini danger delete-task" data-id="${t.id}">Delete</button></div></div>`).join('');return `<section class="card wide"><div class="card-head"><div><h2>Household & School</h2><div class="meta">Recurring chores reset after completion instead of living on the calendar.</div></div><button id="addTask">＋ Task</button></div><div class="actions" style="margin-top:12px">${['All','Richard','Timothy','Olivia','Everyone'].map(x=>`<button class="task-filter ${taskFilter===x?'primary':''}" data-filter="${x}">${x}</button>`).join('')}</div></section><section class="card"><div class="card-head"><div><h2>Household</h2><div class="meta">${due.length} due now · ${upcoming.length} upcoming</div></div></div><div class="chore-subhead">Due now</div>${rows(due)||'<p class="muted">Nothing due right now 🎉</p>'}<div class="chore-subhead">Upcoming</div>${rows(upcoming)||'<p class="muted">No upcoming chores.</p>'}</section><section class="card"><h2>School & Activities</h2>${rows(school)||'<p class="muted">No school or activity tasks for this filter.</p>'}</section><section class="card full"><div class="card-head"><div><h2>Recent Household Completions</h2><div class="meta">Completion history for recurring chores</div></div></div><div class="chore-history">${recent.map(x=>`<div class="history-row"><span>✓</span><div><b>${x.t.name}</b><div class="meta">${fmtDate(x.ds)} · ${x.t.assigned||'Everyone'}</div></div></div>`).join('')||'<p class="muted">Completed recurring chores will appear here.</p>'}</div></section>`}
