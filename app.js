...
function eventOccursOn(e,ds){if(e.date===ds)return true;if(!e.repeat||e.repeat==='None'||ds<e.date)return false;if(e.repeatUntil&&ds>e.repeatUntil)return false;let a=new Date(e.date+'T12:00'),b=new Date(ds+'T12:00'),days=Math.round((b-a)/86400000);if(days<0)return false;if(e.repeat==='Daily')return true;if(e.repeat==='Weekly')return days%7===0;if(e.repeat==='Every 2 weeks')return days%14===0;if(e.repeat==='Monthly')return b.getDate()===a.getDate();return false}
...
function bind(){document.querySelectorAll('.meal-shop').forEach(x=>{x.onclick=()=>mealToShopping(Number(x.dataset.id))});document.querySelectorAll('.edit-meal').forEach(x=>{x.onclick=()=>editMeal(Number(x.dataset.id))});document.querySelectorAll('.delete-meal').forEach(x=>{x.onclick=()=>{if(confirm('Delete this meal?')){data.meals=data.meals.filter(m=>m.id!=x.dataset.id);save();render()}}});document.querySelectorAll('.edit-task').forEach(x => {
