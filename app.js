...
function resetRecurringTasks(){data.settings=data.settings||{};let today=iso(new Date());data.tasks.forEach(t=>{if(!t.repeat||t.repeat==='None'||!t.due||!t.done)return;let due=new Date(t.due+'T12:00'),now=new Date(today+'T12:00'),next=new Date(due);if(t.repeat==='Daily')next.setDate(next.getDate()+1);if(t.repeat==='Weekly')next.setDate(next.getDate()+7);if(t.repeat==='Monthly')next.setMonth(next.getMonth()+1);if(next<=now){while(next<=now){if(t.repeat==='Daily')next.setDate(next.getDate()+1);else if(t.repeat==='Weekly')next.setDate(next.getDate()+7);else if(t.repeat==='Monthly')next.setMonth(next.getMonth()+1)}t.due=iso(next);t.done=false}})}
...
function editDashboard(){data.settings=data.settings||{};data.settings.homeCards=data.settings.homeCards||{coming:true,weather:true,week:true,shopping:true,meals:true,payments:true};let labels={coming:'Coming up',weather:'Weather',week:'Week at a glance',shopping:'Shopping',meals:'Meals this week',payments:'Payments'};$('#modalTitle').textContent='Edit dashboard';$('#modalBody').innerHTML=`<p class="meta">Choose which cards appear on Home. You can change this anytime.</p>${Object.entries(labels).map(([k,v])=>`<label class="item"><input type="checkbox" class="dash-card" data-card="${k}" ${data.settings.homeCards[k]!==false?'checked':''}><div style="flex:1"><b>${v}</b></div></label>`).join('')}<div class="modal-actions"><span></span><button type="button" class="primary" id="dashSave">Save dashboard</button></div>`;$('#modal').showModal();$('#dashSave').onclick=()=>{document.querySelectorAll('.dash-card').forEach(x=>data.settings.homeCards[x.dataset.card]=x.checked);save();$('#modal').close();render()}}
function home(){let hc=(data.settings&&data.settings.homeCards)||{};let ev=data.events.filter(e=>e.date>=iso(new Date())).sort((a,b)=>(a.date+a.time).localeCompare(b.date+b.time)).slice(0,6);return `${hc.coming!==false?`<section class="card wide"><div class="card-head"><h2>Coming up</h2><span class="pill">Next 7 days</span></div>${ev.map(eventRow).join('')}</section>`:''}${hc.weather!==false?`<section class="card"><h2>Weather</h2><div class="weather-row"><div class="weather" data-weather="Las Vegas">Loading…</div><div class="weather" data-weather="Flagstaff">Loading…</div></div></section>`:''}${hc.week!==false?`<section class="card full"><div class="card-head"><h2>Week at a glance</h2><button data-go="calendar">Open calendar</button></div>${weekStrip()}</section>`:''}${hc.shopping!==false?`<section class="card"><div class="card-head"><h2>Shopping</h2><span class="pill">${data.staples.filter(x=>x.need).length} needed</span></div>${data.staples.filter(x=>x.need).slice(0,5).map(x=>`<div class="item">☐ <div>${x.name}${x.qty>1?` ×${x.qty}`:''}</div></div>`).join('')}<div class="actions"><button data-go="shop">Review staples</button></div></section>`:''}${hc.meals!==false?`<section class="card"><h2>Meals this week</h2>${mealPlanRows()}<div class="actions"><button data-go="meals">Plan week</button></div></section>`:''}${hc.payments!==false?`<section class="card"><h2>Payments</h2>${data.payments.filter(p=>!p.paid).slice(0,4).map(p=>`<div class="item"><div><b>${p.name}</b><div class="meta">Due ${p.day}${p.amount?` · $${p.amount}`:''}</div></div></div>`).join('')}</section><section class="card"><h2>Household</h2>${data.tasks.slice(0,4).map(t=>`<div class="item"><input class="check task-check" data-id="${t.id}" type="checkbox" ${t.done?'checked':''}><div>${t.name}<div class="meta">${fmtDate(t.due)} · ${t.repeat}</div></div></div>`).join('')}</section>`:''}`}
...
function bind(){document.querySelectorAll('.cal-event').forEach(x=>{x.onclick=()=>openCalendarEvent(Number(x.dataset.id),x.dataset.date)});document.querySelectorAll('.meal-shop').forEach(x=>{x.onclick=()=>mealToShopping(Number(x.dataset.id))});document.querySelectorAll('.edit-meal').forEach(x=>{x.onclick=()=>editMeal(Number(x.dataset.id))});document.querySelectorAll('.delete-meal').forEach(x=>{x.onclick=()=>{if(confirm('Delete this meal?')){data.meals=data.meals.filter(m=>m.id!=x.dataset.id);save();render()}}});document.querySelectorAll('.edit-task').forEach(x => {
