...
function openCalendarEvent(id,ds){let e=data.events.find(x=>x.id==id);if(!e)return;if(!e.repeat||e.repeat==='None'||ds===e.date){editEvent(id);return}$('#modalTitle').textContent=e.title;$('#modalBody').innerHTML=`<p>This is part of a recurring event. What would you like to do?</p><div class="actions"><button type="button" id="editOne">Edit this event only</button><button type="button" class="primary" id="editFuture">Edit this and future</button><button type="button" class="danger" id="deleteOccurrence">Delete…</button></div>`;$('#modal').showModal();$('#deleteOccurrence').onclick=()=>{$('#modal').close();deleteEventChoice(id,ds)};$('#editOne').onclick=()=>{let copy={...e,id:Date.now(),date:ds,repeat:'None',repeatUntil:'',excludedDates:[]};e.excludedDates=e.excludedDates||[];if(!e.excludedDates.includes(ds))e.excludedDates.push(ds);data.events.push(copy);save();$('#modal').close();editEvent(copy.id)};$('#editFuture').onclick=()=>{let dayBefore=new Date(ds+'T12:00');dayBefore.setDate(dayBefore.getDate()-1);let copy={...e,id:Date.now(),date:ds,excludedDates:[]};e.repeatUntil=iso(dayBefore);data.events.push(copy);save();$('#modal').close();editEvent(copy.id)}}
