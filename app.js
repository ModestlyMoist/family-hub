...
let data=JSON.parse(localStorage.getItem('familyHubData')||'null')||SAMPLE; let view='home'; let calendarMode='month'; let taskFilter='All'; let calCursor=new Date(); let habitCursor=weekStart(); let habitMode='week'; calCursor.setDate(1); let activePin=sessionStorage.getItem('familyHubPin')||localStorage.getItem('familyHubPin')||'';
...
function weekHabitStats(startDs=weekStart()){let days=habitDays(startDs),done=0,total=0;days.forEach(ds=>data.habits.filter(h=>(h.trackerType||'habit')==='habit').forEach(h=>{if(habitApplies(h,ds)){total++;if(data.habitLog[ds]?.[h.id])done++}}));return {done,total,pct:total?Math.round(done/total*100):0}}
