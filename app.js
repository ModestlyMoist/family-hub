...
function habitSummaryView(){let starts=Array.from({length:4},(_,i)=>{let d=new Date(weekStart()+'T12:00');d.setDate(d.getDate()-i*7);return iso(d)}),weeks=starts.map((st,i)=>{let s=weekHabitStats(st);return {...s,start:st,label:i===0?'This week':i===1?'Last week':new Date(st+'T12:00').toLocaleDateString([],{month:'short',day:'numeric'})}}),trackerWeek=(h,st)=>{let total=0,done=0;habitDays(st).forEach(ds=>{if(habitApplies(h,ds)){total++;if(data.habitLog[ds]?.[h.id])done++}});return {total,done,pct:total?Math.round(done/total*100):null}},rows=data.habits.map(h=>{let ws=starts.map(st=>trackerWeek(h,st)),allTotal=0,allDone=0;Object.keys(data.habitLog).forEach(ds=>{if(habitApplies(h,ds)){allTotal++;if(data.habitLog[ds]?.[h.id])allDone++}});return {h,ws,total:allTotal,done:allDone,pct:allTotal?Math.round(allDone/allTotal*100):0}}),delta=weeks[0].pct-weeks[1].pct;return `<section class="card full"><div class="card-head"><div><h2>Habit summary</h2><div class="meta">4-week progress and GG earnings</div></div><button id="habitBack">Back to week</button></div><div class="habit-summary-stats"><div><span>This week</span><strong>${weeks[0].pct}%</strong><small>${weeks[0].done} of ${weeks[0].total} · ${delta===0?'same as last week':(delta>0?'+':'')+delta+' pts vs last week'}</small></div><div><span>Last week</span><strong>${weeks[1].pct}%</strong><small>${weeks[1].done} of ${weeks[1].total}</small></div><div><span>Total GG</span><strong>${ggPoints().toFixed(2)}</strong><small>points earned</small></div></div><div class="four-week">${weeks.map(w=>`<div><span>${w.label}</span><b>${w.pct}%</b><small>+${w.points.toFixed(2)} GG</small><div class="trend-bar"><i style="width:${w.pct}%"></i></div></div>`).join('')}</div></section><section class="card full"><h2>By tracker · last 4 weeks</h2>${rows.map(r=>`<div class="tracker-trend"><div class="tracker-trend-name"><b>${r.h.name}</b><span class="pill">${r.h.trackerType==='checkin'?'Check-in':'Habit'}</span></div><div class="tracker-weeks">${r.ws.map((w,i)=>`<div><small>${weeks[i].label}</small><strong>${w.pct===null?'—':w.pct+'%'}</strong></div>`).join('')}</div><div class="meta">${r.h.trackerType==='checkin'?`Overall: happened ${r.done} of ${r.total} tracked days`:`Overall: ${r.done} of ${r.total} completed`}</div></div>`).join('')||'<p class="muted">No tracking history yet.</p>'}</section>`}
