...
function tasksView(){let all=data.tasks.filter(t=>taskFilter==='All'||(t.assigned||'Everyone')===taskFilter),today=iso(new Date()),weekEnd=new Date(weekStart()+'T12:00');weekEnd.setDate(weekEnd.getDate()+6);let nextDue=t=>{if(t.taskMode!=='cycle')return t.due||'';if(t.cycle==='Pick the Days'||t.cycleStart)return nextTaskDue(t,today);let n=choreIntervalDays(t);if(!n)return '';if(!t.lastCompleted)return today;let d=new Date(t.lastCompleted+'T12:00');d.setDate(d.getDate()+n);return iso(d)},groups=[{title:'Household',match:t=>t.category==='Household'||t.category==='Chore',empty:'household chores'},{title:'School + Activities',match:t=>['School','Sports'].includes(t.category),empty:'school or activity tasks'},{title:'Work',match:t=>t.category==='Work',empty:'work tasks'}],rows=arr=>arr.map(t=>`<div class="item"><input class="check task-check" data-id="${t.id}" type="checkbox" ${t.done?'checked':''}><div style="flex:1"><b>${t.name}</b><div class="meta">${t.taskMode==='cycle'&&nextDue(t)>today?'Next: '+fmtDate(nextDue(t))+' · '+(t.cycle||'Repeating'):choreStatus(t)} · ${t.assigned||'Everyone'}</div></div><div class="item-actions"><button class="mini edit-task" data-id="${t.id}">Edit</button><button class="mini danger delete-task" data-id="${t.id}">Delete</button></div></div>`).join(''),master=arr=>arr.map(t=>`<div class="item"><div style="flex:1"><b>${t.name}</b><div class="meta">${t.taskMode==='cycle'?(t.cycle||'Repeating'):(t.due?fmtDate(t.due):'No due date')} · ${t.assigned||'Everyone'}</div></div><div class="item-actions"><button class="mini edit-task" data-id="${t.id}">Edit</button><button class="mini danger delete-task" data-id="${t.id}">Delete</button></div></div>`).join(''),section=g=>{let items=all.filter(g.match),due=items.filter(t=>!t.done&&((t.taskMode==='cycle'&&nextDue(t)<=today)||(t.taskMode!=='cycle'&&t.due&&t.due<=today))),week=items.filter(t=>{let nd=nextDue(t);return !t.done&&nd>today&&nd<=iso(weekEnd)});let cls=g.title==='Household'?'household':g.title.startsWith('School')?'school':'work';return `<section class="task-area task-area-${cls}"><div class="task-area-head"><h2>${g.title}</h2><span class="meta">${items.length} total</span></div><div class="task-area-top"><div class="card"><div class="card-head"><h3>Due Now</h3><span class="pill">${due.length}</span></div>${rows(due)||'<p class="muted">Nothing due today 🎉</p>'}</div><div class="card"><div class="card-head"><h3>This Week</h3><span class="pill">${week.length}</span></div>${rows(week)||'<p class="muted">Nothing else due this week.</p>'}</div></div><div class="card full task-master"><div class="card-head"><div><h3>${g.title} List</h3><div class="meta">All ${g.empty}</div></div></div>${master(items)||`<p class="muted">No ${g.empty} for this filter.</p>`}</div></section>`};return `<section class="card full"><div class="card-head"><div><h2>Tasks</h2><div class="meta">Household, school, activities & work</div></div><button id="addTask">＋ Task</button></div><div class="actions" style="margin-top:12px">${['All','Richard','Timothy','Olivia','Everyone'].map(x=>`<button class="task-filter ${taskFilter===x?'primary':''}" data-filter="${x}">${x}</button>`).join('')}</div></section>${groups.map(section).join('')}`}
