...
function render(){
  ...
  resetRecurringTasks();
  let now = new Date();$('#todayLabel').textContent=now.toLocaleDateString([],{weekday:'long',month:'long',day:'numeric'});let today=iso(now),todayEvents=eventsForDate(today).length,need=data.staples.filter(x=>x.need).length,dueChores=data.tasks.filter(t=>t.taskMode==='cycle'&&choreStatus(t).startsWith('Due now')).length;let habitsToday=data.habits.filter(h=>(h.trackerType||'habit')==='habit'&&habitApplies(h,today)&&!data.habitLog[today]?.[h.id]).length;$('#summary').textContent=`${todayEvents} event${todayEvents===1?'':'s'} today · ${dueChores} chore${dueChores===1?'':'s'} due · ${habitsToday} habit${habitsToday===1?'':'s'} to complete · ${need} grocer${need===1?'y':'ies'} needed`; let root=$('#dashboard');root.innerHTML=view==='home'?home():view==='calendar'?calendarView():view==='tasks'?tasksView():view==='shop'?shopView():view==='meals'?mealsView():view==='habits'?habitsView():moreView();$('#editDashboard')&&($('#editDashboard').style.display=view==='home'?'':'none');weather();bind()}
