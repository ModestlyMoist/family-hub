...
let data=JSON.parse(localStorage.getItem('familyHubData')||'null')||SAMPLE; let view='home'; let calendarMode='month'; let calCursor=new Date(); calCursor.setDate(1); let activePin=sessionStorage.getItem('familyHubPin')||localStorage.getItem('familyHubPin')||'';
...
function tasksView(){let due=data.tasks.filter(t=>t.taskMode==='cycle'&&choreStatus(t).startsWith('Due now')),house=data.tasks.filter(t=>t.category==='Household'||t.category==='Chore'),school=data.tasks.filter(t=>!house.includes(t));let rows=arr=>arr.map(t=>`<div class="item"><input class="check task-check" data-id="${t.id}" type="checkbox" ${t.done?'checked':''}><div style="flex:1"><b>${t.name}</b><div class="meta">${choreStatus(t)}${t.category?` · ${t.category}`:''}</div></div><div class="item-actions"><button class="mini edit-task" data-id="${t.id}">Edit</button><button class="mini danger delete-task" data-id="${t.id}">Delete</button></div></div>`).join('');return `<section class="card wide"><div class="card-head"><div><h2>Tasks</h2><div class="meta">Chores, school, activities & reminders</div></div><button id="addTask">＋ Task</button></div>${due.length?`<div style="margin-top:14px"><h3>Due now</h3>${rows(due)}</div>`:''}</section><section class="card"><h2>Household</h2>${rows(house)||'<p class="muted">No household tasks yet.</p>'}</section><section class="card"><h2>School & Activities</h2>${rows(school)||'<p class="muted">No school or activity tasks yet.</p>'}</section>`}
