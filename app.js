...
function habitSummaryView(){let starts=Array.from({length:4},(_,i)=>{let d=new Date(weekStart()+'T12:00');d.setDate(d.getDate()-i*7);return iso(d)}),weeks=starts.map((st,i)=>{let s=weekHabitStats(st),ds=habitDays(st);return {...s,start:st,label:i===0?'This Week':i===1?'Last Week':new Date(st+'T12:00').toLocaleDateString([],{month:'short',day:'numeric'}),range:new Date(ds[0]+'T12:00').toLocaleDateString([],{month:'short',day:'numeric'})+' – '+new Date(ds[6]+'T12:00').toLocaleDateString([],{month:'short',day:'numeric'})}}),trackerWeek=(h,st)=>{let total=0,done=0;habitDays(st).forEach(ds=>{if(habitApplies(h,ds)){total++;if(data.habitLog[ds]?.[h.id])done++}});return {total,done,pct:total?Math.round(done/total*100):null}},rows=data.habits.map(h=>({h,ws:starts.map(st=>trackerWeek(h,st))}));return `<section class="card full summary-overall"><div class="card-head"><div><h2>Overall Progress</h2><div class="meta">Completion rate across the last 4 weeks</div></div><button id="habitBack">Back to week</button></div><div class="week-chart">${weeks.map((w,i)=>`<div class="week-chart-card"><b>${w.label}</b><span class="meta">${w.range}</span><div class="ring" style="--p:${w.pct}"><strong>${w.pct}%</strong></div><span>${w.done} of ${w.total} completed</span></div>`).join('')}</div></section><section class="card full"><div class="card-head"><div><h2>By Tracker · Last 4 Weeks</h2><div class="meta">Completion rate for habits · occurrence rate for check-ins</div></div></div><div class="tracker-chart-head"><span>Tracker</span>${weeks.map(w=>`<span>${w.label}</span>`).join('')}<span>4-week trend</span></div><div class="tracker-chart">${rows.map(r=>{let vals=r.ws.map(w=>w.pct??0),pts=vals.map((v,i)=>`${i*33.33},${36-v*.32}`).join(' ');return `<div class="tracker-chart-row"><div class="tracker-label"><b>${r.h.name}</b><span class="pill">${r.h.trackerType==='checkin'?'Check-in':'Habit'}</span></div>${r.ws.map(w=>`<div class="tracker-bar-cell"><div class="mini-bar"><i style="width:${w.pct??0}%"></i></div><strong>${w.pct===null?'—':w.pct+'%'}</strong></div>`).join('')}<svg class="spark" viewBox="0 0 100 40" preserveAspectRatio="none"><polyline points="${pts}"/></svg></div>`}).join('')||'<p class="muted">No tracking history yet.</p>'}</div></section>`}
