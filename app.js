...
function render(){
  ...
  resetRecurringTasks();
  let now = new Date();$('#todayLabel').textContent=now.toLocaleDateString([],{weekday:'long',month:'long',day:'numeric'});let today=iso(now),todayEvents=eventsForDate(today).length,need=data.staples.filter(x=>x.need).length,dueChores=data.tasks.filter(t=>t.taskMode==='cycle'&&choreStatus(t).startsWith('Due now')).length;let habitsToday=data.habits.filter(h=>(h.trackerType||'habit')==='habit'&&habitApplies(h,today)&&!data.habitLog[today]?.[h.id]).length;$('#summary').textContent=`${todayEvents} event${todayEvents===1?'':'s'} today · ${dueChores} chore${dueChores===1?'':'s'} due · ${habitsToday} habit${habitsToday===1?'':'s'} to complete · ${need} grocer${need===1?'y':'ies'} needed`; let root=$('#dashboard');root.innerHTML=view==='home'?home():view==='today'?todayView():view==='calendar'?calendarView():view==='tasks'?tasksView():view==='shop'?shopView():view==='meals'?mealsView():view==='habits'?habitsView():moreView();$('#editDashboard')&&($('#editDashboard').style.display=view==='home'?'':'none');weather();bind()}
...
function weekMealsToShopping(){let names=[...new Set(habitDays(weekStart()).map(ds=>data.mealPlan[ds]).filter(Boolean))],meals=names.map(n=>data.meals.find(m=>m.name===n)).filter(Boolean),items=[];meals.forEach(m=>(m.ingredients||'').split(/[,\n]/).map(x=>x.trim()).filter(Boolean).forEach(name=>{if(!items.some(x=>x.toLowerCase()===name.toLowerCase()))items.push(name)}));if(!items.length){alert('Add ingredients to your planned meals first.');return}$('#modalTitle').textContent='Add week to shopping';$('#modalBody').innerHTML=`<p class="meta">Choose ingredients from this week’s ${meals.length} planned meal${meals.length===1?'':'s'}.</p><div>${items.map(name=>{let existing=data.staples.some(s=>s.name.trim().toLowerCase()===name.toLowerCase());return `<label class="item"><input type="checkbox" class="week-meal-ing" data-name="${name.replace(/"/g,'&quot;')}" ${existing?'':'checked'}><div style="flex:1"><b>${name}</b><div class="meta">${existing?'Already in shopping list':'New item'}</div></div></label>`}).join('')}</div><div class="modal-actions"><span></span><button type="button" class="primary" id="addWeekIngredientsBtn">Add selected</button></div>`;$('#modal').showModal();$('#addWeekIngredientsBtn').onclick=()=>{document.querySelectorAll('.week-meal-ing:checked').forEach(x=>{let name=x.dataset.name,existing=data.staples.find(s=>s.name.trim().toLowerCase()===name.toLowerCase());if(existing)existing.need=true;else data.staples.push({id:Date.now()+Math.random(),name,qty:1,need:true,permanent:false})});save();$('#modal').close();render()}}
