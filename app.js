...
function tasksView(){let all=data.tasks.filter(t=>taskFilter==='All'||(t.assigned||'Everyone')===taskFilter),house=all.filter(t=>t.category==='Household'||t.category==='Chore'),school=all.filter(t=>!house.includes(t)),today=iso(new Date()),weekEnd=new Date(weekStart()+'T12:00');weekEnd.setDate(weekEnd.getDate()+6);let nextDue=t=>{if(t.taskMode!=='cycle')return t.due||'';let n=choreIntervalDays(t);if(!n)return '';if(!t.lastCompleted)return today;let d=new Date(t.lastCompleted+'T12:00');d.setDate(d.getDate()+n);return iso(d)},due=house.filter(t=>t.taskMode==='cycle'&&nextDue(t)<=today),thisWeek=house.filter(t=>t.taskMode==='cycle'&&nextDue(t)>today&&nextDue(t)<=iso(weekEnd)).sort((a,b)=>nextDue(a).localeCompare(nextDue(b))),datedHouse=house.filter(t=>t.taskMode!=='cycle'),status=t=>{let nd=nextDue(t);if(t.taskMode==='cycle'&&nd&&nd!==today&&nd<=iso(weekEnd))return 'Next: '+fmtDate(nd)+' · '+(t.cycle||'Repeating');return choreStatus(t)},rows=arr=>arr.map(t=>`<div class="item"><input class="check task-check" data-id="${t.id}" type="checkbox" ${t.done?'checked':''}><div style="flex:1"><b>${t.name}</b><div class="meta">${status(t)}${t.category?` · ${t.category}`:''} · ${t.assigned||'Everyone'}</div></div><div class="item-actions"><button class="mini edit-task" data-id="${t.id}">Edit</button><button class="mini danger delete-task" data-id="${t.id}">Delete</button></div></div>`).join(''),listRows=arr=>arr.map(t=>`<div class="item"><div style="flex:1"><b>${t.name}</b><div class="meta">${t.taskMode==='cycle'?(t.cycle||'Repeating'):(t.due?fmtDate(t.due):'One-time')} · ${t.assigned||'Everyone'}</div></div><div class="item-actions"><button class="mini edit-task" data-id="${t.id}">Edit</button><button class="mini danger delete-task" data-id="${t.id}">Delete</button></div></div>`).join('');return `<section class="card full"><div class="card-head"><div><h2>Tasks</h2><div class="meta">Chores, school, activities & reminders</div></div><button id="addTask">＋ Task</button></div><div class="actions" style="margin-top:12px">${['All','Richard','Timothy','Olivia','Everyone'].map(x=>`<button class="task-filter ${taskFilter===x?'primary':''}" data-filter="${x}">${x}</button>`).join('')}</div></section><section class="card"><div class="card-head"><div><h2>Due Now</h2><div class="meta">Household chores due today</div></div><span class="pill">${due.length}</span></div>${rows(due)||'<p class="muted">Nothing due today 🎉</p>'}</section><section class="card"><div class="card-head"><div><h2>This Week</h2><div class="meta">Chores due again before the week ends</div></div><span class="pill">${thisWeek.length}</span></div>${rows(thisWeek)||'<p class="muted">No more recurring chores due this week.</p>'}</section><section class="card"><h2>School & Activities</h2>${rows(school)||'<p class="muted">No school or activity tasks for this filter.</p>'}</section><section class="card full"><div class="card-head"><div><h2>Household Chores</h2><div class="meta">All household chores and their recurrence</div></div></div>${listRows(house)||'<p class="muted">No household chores for this filter.</p>'}</section>`}
