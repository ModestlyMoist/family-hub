...
function render(){
  ...
  resetRecurringTasks();
  let now = new Date();$('#todayLabel').textContent=now.toLocaleDateString([],{weekday:'long',month:'long',day:'numeric'});let today=iso(now),todayEvents=eventsForDate(today).length,need=data.staples.filter(x=>x.need).length,dueChores=data.tasks.filter(t=>t.taskMode==='cycle'&&choreStatus(t).startsWith('Due now')).length;$('#summary').textContent=`${todayEvents} event${todayEvents===1?'':'s'} today · ${dueChores} chore${dueChores===1?'':'s'} due · ${need} grocer${need===1?'y':'ies'} needed`; let root=$('#dashboard');root.innerHTML=view==='home'?home():view==='calendar'?calendarView():view==='tasks'?tasksView():view==='shop'?shopView():view==='meals'?mealsView():view==='habits'?habitsView():moreView();weather();bind()}
...
function habitDays(){let start=new Date(weekStart()+'T12:00');return Array.from({length:7},(_,i)=>{let d=new Date(start);d.setDate(start.getDate()+i);return iso(d)})}
function habitApplies(h,ds){let dow=new Date(ds+'T12:00').getDay();return !h.days||h.days.length===0||h.days.includes(dow)}
function habitsView(){let days=habitDays(),done=0,total=0;days.forEach(ds=>data.habits.forEach(h=>{if(habitApplies(h,ds)){total++;if(data.habitLog[ds]?.[h.id])done++}}));let pct=total?Math.round(done/total*100):0;return `<section class="card full"><div class="card-head"><div><h2>Weekly habits</h2><div class="meta">${pct}% complete this week · progress is saved week to week</div></div><div class="actions"><button id="habitSummary">Summary</button><button id="addHabit">＋ Habit</button></div></div><div class="habit-week">${days.map(ds=>{let dt=new Date(ds+'T12:00'),today=ds===iso(new Date()),hs=data.habits.filter(h=>habitApplies(h,ds));return `<div class="habit-day ${today?'habit-today':''}"><div class="habit-day-head"><b>${today?'Today':dt.toLocaleDateString([],{weekday:'short'})}</b><span>${dt.toLocaleDateString([],{month:'short',day:'numeric'})}</span></div>${hs.map(h=>`<label class="habit-check"><input type="checkbox" class="habit-toggle" data-id="${h.id}" data-date="${ds}" ${data.habitLog[ds]?.[h.id]?'checked':''}><span>${h.name}</span></label>`).join('')||'<div class="meta">No habits</div>'}</div>`}).join('')}</div></section><section class="card full"><div class="card-head"><div><h2>Habit setup</h2><div class="meta">Daily habits appear every day; custom habits only appear on selected days.</div></div></div>${data.habits.map(h=>`<div class="item"><div style="flex:1"><b>${h.name}</b><div class="meta">${!h.days||h.days.length===0?'Every day':h.days.map(d=>['Sun','Mon','Tue','Wed','Thu','Fri','Sat'][d]).join(', ')}</div></div><div class="item-actions"><button class="mini habit-up" data-id="${h.id}" title="Move up">↑</button><button class="mini habit-down" data-id="${h.id}" title="Move down">↓</button><button class="mini edit-habit" data-id="${h.id}">Edit</button></div></div>`).join('')||'<p class="muted">Add the first habit to start tracking.</p>'}</section>`}
function editHabit(id){let h=data.habits.find(x=>x.id==id)||{id:Date.now(),name:'',days:[]},fresh=!data.habits.includes(h);$('#modalTitle').textContent=fresh?'Add habit':'Edit habit';$('#modalBody').innerHTML=`<div class="form-grid"><label class="field full">Habit<input id="heName" value="${h.name||''}" placeholder="Read 20 minutes"></label><label class="field full">Schedule<select id="heSchedule"><option value="daily" ${!h.days||!h.days.length?'selected':''}>Every day</option><option value="custom" ${h.days?.length?'selected':''}>Specific days</option></select></label><div class="field full" id="habitDaysPick" style="${h.days?.length?'':'display:none'}"><div class="habit-day-picks">${[['Mon',1],['Tue',2],['Wed',3],['Thu',4],['Fri',5],['Sat',6],['Sun',0]].map(([n,d])=>`<label><input type="checkbox" value="${d}" ${h.days?.includes(d)?'checked':''}> ${n}</label>`).join('')}</div></div></div><div class="modal-actions">${fresh?'<span></span>':'<button type="button" class="danger" id="heDelete">Delete habit</button>'}<button type="button" class="primary" id="heSave">Save habit</button></div>`;$('#modal').showModal();$('#heSchedule').onchange=()=>$('#habitDaysPick').style.display=$('#heSchedule').value==='custom'?'':'none';$('#heSave').onclick=()=>{h.name=$('#heName').value.trim();if(!h.name)return;h.days=$('#heSchedule').value==='daily'?[]:[...document.querySelectorAll('#habitDaysPick input:checked')].map(x=>+x.value);if($('#heSchedule').value==='custom'&&!h.days.length){alert('Choose at least one day.');return}if(fresh)data.habits.push(h);save();$('#modal').close();render()};$('#heDelete')&&($('#heDelete').onclick=()=>{if(confirm('Delete this habit? Past completion history will remain stored.')){data.habits=data.habits.filter(x=>x.id!=id);save();$('#modal').close();render()}})}
function habitSummary(){let rows=data.habits.map(h=>{let total=0,done=0;Object.keys(data.habitLog).sort().forEach(ds=>{if(habitApplies(h,ds)){total++;if(data.habitLog[ds]?.[h.id])done++}});return {h,total,done,pct:total?Math.round(done/total*100):0}});$('#modalTitle').textContent='Habit summary';$('#modalBody').innerHTML=`<p class="meta">Completion history across all saved weeks.</p>${rows.map(r=>`<div class="habit-summary-row"><div><b>${r.h.name}</b><div class="meta">${r.done} of ${r.total} completed</div></div><strong>${r.pct}%</strong></div>`).join('')||'<p class="muted">No habits to summarize yet.</p>'}`;$('#modal').showModal()}
function moreView(){
  return `
    <section class="card">
