...
function isHome(){return !!R()?.querySelector('[data-dashboard-home]')}
...
function savedOrder(pairs){data.settings=data.settings||{};let ids=pairs.map(x=>x[0]),saved=window.familyHubDashboard?window.familyHubDashboard.prefs().order:(Array.isArray(data.settings.dashboardOrderAll)?data.settings.dashboardOrderAll:[]);let o=saved.filter(x=>ids.includes(x));ids.forEach(x=>{if(!o.includes(x))o.push(x)});return o}
...
function move(id,dir){let r=R(),pairs=getTiles(r),o=savedOrder(pairs),i=o.indexOf(id),j=i+Number(dir);if(i<0||j<0||j>=o.length)return;[o[i],o[j]]=[o[j],o[i]];data.settings.dashboardOrderAll=o;if(data.settings.dashboard)data.settings.dashboard.order=o.slice();localSave();syncSave();applyVisualOrder()}
