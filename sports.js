...
(function(){
  ...
  function defaultTeam(person){return person==='Richard'?'SSLL AA Mariners':''}
  ...
  function pageMarkup(){
    ensure();let ev=[...sports()].sort((a,b)=>(a.date+(a.time||'')).localeCompare(b.date+(b.time||''))),today=vegasToday(),up=ev.filter(e=>e.date>=today),nextR=up.find(e=>e.person==='Richard'||e.person==='Both'),nextT=up.find(e=>e.person==='Timothy'||e.person==='Both');
    let kid=(name,next)=>'<section class="card sport-kid"><div class="card-head"><div><div class="eyebrow">'+name.toUpperCase()+'</div><h2>'+(name==='Richard'?'SSLL AA Mariners':name+' Sports')+'</h2></div></div>'+(next?'<div class="sport-next"><span>Next up</span><strong>'+esc(next.title)+'</strong><div class="meta">'+fmt(next.date)+' · '+(next.time||'All day')+(next.arrival?' · Arrive '+next.arrival:(next.leave?' · Leave '+next.leave:''))+'</div>'+(next.location?'<div class="meta">'+esc(next.location)+'</div>':'')+(next.bring?'<div class="sport-bring">Bring: '+esc(next.bring)+'</div>':'')+'</div>':'<p class="muted">No upcoming sports scheduled.</p>')+'</section>';
    return '<div class="sports-page-full"><section class="sports-page-head"><div><div class="eyebrow">FAMILY SPORTS</div><h1>Sports Hub</h1><p class="meta">Practices, games, schedules and reminders for Richard & Timothy.</p></div><button class="primary" data-sport-add>＋ Sports event</button></section><div class="sports-stats"><div><span>Upcoming</span><strong>'+up.length+'</strong></div><div><span>Practices</span><strong>'+up.filter(e=>e.type==='Practice').length+'</strong></div><div><span>Games</span><strong>'+up.filter(e=>e.type==='Game').length+'</strong></div></div><div class="sports-kids">'+kid('Richard',nextR)+kid('Timothy',nextT)+'</div><section class="card full sports-schedule-card"><div class="card-head"><div><h2>Upcoming Schedule</h2><div class="meta">Events here also appear on the Family Calendar.</div></div><div class="sports-filters"><button class="active" data-sport-filter="All">All</button><button data-sport-filter="Richard">Richard</button><button data-sport-filter="Timothy">Timothy</button></div></div><div class="sports-list">'+(up.length?up.map(e=>row(e)).join(''):'<p class="muted">No upcoming practices or games.</p>')+'</div></section></div>';
  }
  function open(){
  ...
  function row(e){return '<div class="sport-row" data-sport-person="'+esc(e.person)+'"><span class="sport-dot" style="background:'+personColor(e.person)+'"></span><div class="sport-info"><div class="sport-title"><b>'+esc(e.title)+'</b><span class="pill">'+esc(e.type)+'</span></div><div class="meta">'+fmt(e.date)+' · '+(e.time||'All day')+(e.end?'–'+e.end:'')+(e.arrival?' · Arrive '+e.arrival:(e.leave?' · Leave '+e.leave:''))+'</div><div class="meta">'+esc(e.person)+(e.team?' · '+esc(e.team):'')+(e.opponent?' · '+(e.homeAway==='Away'?'@ ':'vs ')+esc(e.opponent):'')+(e.location?' · '+esc(e.location):'')+'</div></div><div class="item-actions"><button class="mini" data-sport-edit="'+e.id+'">Edit</button></div></div>'}
  function form(id){
    ...
    $('#modal').showModal();
    const personSelect=$('#spPerson'),teamInput=$('#spTeam');
    ...
    teamInput.dataset.autoTeam=defaultTeam(personSelect.value);
