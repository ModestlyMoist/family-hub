...
(function(){
  ...
  function pageMarkup(){
    ...
    return '<div class="sports-page-full"><section class="sports-page-head"><div><div class="eyebrow">FAMILY SPORTS</div><h1>Sports Hub</h1><p class="meta">Practices, games, schedules and reminders for Richard & Timothy.</p></div><button class="primary" data-sport-add>＋ Sports event</button></section><div class="sports-stats"><div><span>Upcoming</span><strong>'+up.length+'</strong></div><div><span>Practices</span><strong>'+up.filter(e=>e.type==='Practice').length+'</strong></div><div><span>Games</span><strong>'+up.filter(e=>e.type==='Game').length+'</strong></div></div><div class="sports-kids" style="display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:10px;width:100%">'+kid('Richard',nextR)+kid('Timothy',nextT)+'</div><section class="card full sports-schedule-card"><div class="card-head"><div><h2>Upcoming Schedule</h2><div class="meta">Events here also appear on the Family Calendar.</div></div><div class="sports-filters"><button class="active" data-sport-filter="All">All</button><button data-sport-filter="Richard">Richard</button><button data-sport-filter="Timothy">Timothy</button></div></div><div class="sports-list">'+(up.length?up.map(e=>row(e)).join(''):'<p class="muted">No upcoming practices or games.</p>')+'</div></section></div>';
  }
  function open(){
