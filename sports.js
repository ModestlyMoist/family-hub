...
(function(){
  ...
  function defaultTeam(person){return person==='Richard'?'SSLL AA Mariners':person==='Timothy'?'SSLL Teeball Majors Giants':''}
  ...
  function pageMarkup(){
    ...
    let kid=(name,next)=>{let rec=record(name);return '<section class="sport-kid" style="flex:1 1 0;min-width:0;background:var(--card);border:1px solid var(--line);border-radius:22px;padding:15px;box-shadow:var(--shadow)"><div class="card-head"><div><div class="eyebrow">'+name.toUpperCase()+'</div><h2>'+(name==='Richard'?'SSLL AA Mariners':name==='Timothy'?'SSLL Teeball Majors Giants':name+' Sports')+'</h2></div><div class="sport-record"><span>Record</span><strong>'+rec.w+'–'+rec.l+(rec.t?'–'+rec.t:'')+'</strong></div></div>'+(next?'<div class="sport-next"><span>Next up</span><strong>'+esc(next.title)+'</strong><div class="meta">'+fmt(next.date)+' · '+window.familyHubTime.format(next.time)+(next.arrival?' · Arrive '+window.familyHubTime.format(next.arrival):(next.leave?' · Leave '+window.familyHubTime.format(next.leave):''))+'</div>'+(next.location?'<div class="meta">'+esc(next.location)+'</div>':'')+(next.bring?'<div class="sport-bring">Bring: '+esc(next.bring)+'</div>':'')+'</div>':'<p class="muted">No upcoming sports scheduled.</p>')+'</section>'};
  ...
  function row(e){let past=e.type==='Game'&&e.date<vegasToday(),result=e.result?'<span class="sport-result '+e.result.toLowerCase()+'">'+e.result+(e.teamScore!==undefined&&e.teamScore!==''?' '+e.teamScore+'–'+e.opponentScore:'')+'</span>':'';return '<div class="sport-row" data-sport-date="'+esc(e.date)+'" data-sport-id="'+e.id+'" data-sport-person="'+esc(e.person)+'"><span class="sport-dot" style="background:'+personColor(e.person)+'"></span><div class="sport-info"><div class="sport-title"><b>'+esc(e.title)+'</b><span class="pill">'+esc(e.type)+'</span>'+result+'</div><div class="meta">'+fmt(e.date)+' · '+window.familyHubTime.range(e.time,e.end)+(e.arrival?' · Arrive '+window.familyHubTime.format(e.arrival):(e.leave?' · Leave '+window.familyHubTime.format(e.leave):''))+'</div><div class="meta">'+esc(e.person)+(e.team?' · '+esc(e.team):'')+(e.opponent?' · '+(e.homeAway==='Away'?'@ ':'vs ')+esc(e.opponent):'')+(e.location?' · '+esc(e.location):'')+'</div></div><div class="item-actions">'+(past&&!e.result?'<button class="primary mini" data-sport-score="'+e.id+'">Add score</button>':'')+'<button class="mini" data-sport-edit="'+e.id+'">Edit</button></div></div>'}
  function form(id){
  ...
  window.openSportsHub=open;document.addEventListener('click',e=>{
