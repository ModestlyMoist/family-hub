...
(function(root){
 ...
 function summarize(data,calc,month){
  ...
  const attention=cats.filter(c=>!c.creditAccountId).map(c=>({id:String(c.id),name:c.name,group:c.group,available:calc.available(c.id,month),gap:round(Math.max(0,(Number(c.expectedMonthly)||0)-(Number(b.monthly?.[month]?.assignments?.[c.id])||0)))})).filter(c=>c.available<0||c.gap>0);
