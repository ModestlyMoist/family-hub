...
(function(){
  ...
  function history(){
    ...
    let smokingDays=rows.filter(x=>x.h>0).length;let avgHits=smokingDays?(hits/smokingDays).toFixed(1):'0.0';
    ...
    return '<section class="card full wellness-history-card"><div class="card-head"><div><h2>Wellness History</h2><div class="meta">Mood and smoking trends over time</div></div><div class="wellness-history-ranges">'+
      ...
      '</div></div><div class="wellness-history-stats"><div><span>Average mood</span><strong>'+avg+(avg==='—'?'':' / 10')+'</strong></div><div><span>Best mood</span><strong>'+best+(best==='—'?'':' / 10')+'</strong></div><div><span>Total hits</span><strong>'+hits+'</strong></div><div><span>Smoking days</span><strong>'+smokingDays+'</strong></div><div><span>Average hits</span><strong>'+avgHits+'</strong></div></div>'+
      '<details class="wellness-history-breakdown"><summary><span>By-date breakdown</span><span class="wellness-history-chevron">⌄</span></summary><div class="wellness-history-list">'+(recent.length?recent.map(x=>'<div class="wellness-history-row"><div><b>'+new Date(x.d+'T12:00').toLocaleDateString([],{weekday:'short',month:'short',day:'numeric'})+'</b></div><div><span>Mood</span><strong>'+(x.m||'—')+'</strong></div><div><span>Hits</span><strong>'+x.h+'</strong></div></div>').join(''):'<p class="muted">No wellness entries yet.</p>')+'</div></details></section>';
  }
