...
(function(){
 ...
 function calculate(excludeId,throughDate){
  ...
  months.forEach(function(m){
   ...
   (data.budgetExpenses||[]).filter(function(x){return String(x.date||'').slice(0,7)===m&&(!throughDate||String(x.date)<=throughDate)}).forEach(function(x){var cat=b.categories.find(function(c){return String(c.name).toLowerCase()===String(x.category).toLowerCase()});if(cat)add(cat.id,-cents(x.amount))});
   tx.filter(function(x){return String(x.date||'').slice(0,7)===m&&(!throughDate||String(x.date)<=throughDate)}).forEach(function(x){
 ...
 function prepare(x,old){
  ...
  if(!/^\d{4}-\d{2}-\d{2}$/.test(x.date)||!Number.isFinite(Number(x.amount))||cents(x.amount)<=0)return 'Enter a valid date and amount.';
