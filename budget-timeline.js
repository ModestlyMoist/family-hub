...
(function(){
  ...
  function card(label,start,end,income,current){
    var bills=((typeof data!=='undefined'&&data.payments)||[]).map(function(p){return {p:p,date:billDate(p,start,end)}}).filter(function(x){return x.date}).sort(function(a,b){return a.date.localeCompare(b.date)});
    var total=bills.reduce(function(s,x){return s+(Number(x.p.amount)||0)},0);var days=Math.round((new Date(end+'T12:00')-new Date(start+'T12:00'))/86400000)+1;var debtMonthly=((typeof data!=='undefined'&&data.debts)||[]).reduce(function(s,d){return s+(Number(d.minimum)||0)},0);var debtReserve=debtMonthly*(days/30.4375);var expenses=((typeof data!=='undefined'&&data.budgetExpenses)||[]).filter(function(x){return x.date>=start&&x.date<=end}).reduce(function(s,x){return s+(Number(x.amount)||0)},0);var committed=total+debtReserve+expenses;var free=Number(income||0)-committed;
