...
(async()=>{
 ...
 // Same-field edits must stop before POST and preserve both review values.
 const conflictBase=state(false),conflictLocal=state(false),cloud=state(false);conflictLocal.budget.monthly['2026-09'].assignments.disney=100;cloud.budget.monthly['2026-09'].assignments.disney=200;r=runtime(conflictLocal,conflictBase,cloud);let conflictResponse=await r.save();assert.equal(conflictResponse.status,409);assert.equal(r.postCount,0);assert.equal(r.ctx.window.familyHubSyncConflict.fields[0].local,100);assert.equal(r.ctx.window.familyHubSyncConflict.fields[0].remote,200);assert.ok(JSON.parse(r.storage.get('familyHubBackups')).length);
