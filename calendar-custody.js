// Calendar 2.0 — custody handoff improvements v241a
(function(){
  ...
  function installSummary(){
    ...
    let box=document.createElement('button');box.type='button';box.className='custody-next-handoff';box.dataset.handoffDate=h.date;box.innerHTML=`<span class="custody-handoff-icon" style="--handoff-color:${color}">↔</span><span><small>Next custody handoff</small><b>${esc(h.event.title||('To '+dest))}</b></span><span class="custody-handoff-when"><b>${when}</b><small>${time||'All day'}</small></span><span class="custody-handoff-arrow">›</span>`;
    let filters=card.querySelector('.calendar-person-filters'),head=card.querySelector('.card-head');(filters||head)?.insertAdjacentElement('afterend',box);
  }
  function markDays(){
  ...
  function enhanceDayFocus(){
    ...
    let title=document.querySelector('#modalTitle')?.textContent||'';
