...
const ctx={data:local,Headers,Response,setTimeout:fn=>fn(),document:{querySelector:()=>null},localStorage:{getItem:k=>storage.get(k)||null,setItem:(k,v)=>storage.set(k,v),removeItem:k=>storage.delete(k)},window:{addEventListener(){},fetch:async(input,init)=>{if(init?.method==='POST'){posted=JSON.parse(init.body);return new Response('{}',{status:200})}return new Response(JSON.stringify({data:base}),{status:200})}}};
...
(async()=>{
 ...
 assert.equal(separate.window.testMerge(saved.budget.transactions,[],saved.budget.transactions,{'tx-local':'deleted'}).length,0);
